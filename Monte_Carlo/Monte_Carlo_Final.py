...
real_difference = abs((sum(grades_female)/n_females)-(sum(grades_male)/n_males))
...
for i in range(iter):
    ...
    gender_difference = abs((sum(g_m)/n_m)-(sum(g_f)/n_f))
...
#plt.hist(monte_carlo_difference,bins = 'auto', range=[-2.5,2.5])
#plt.axvline(real_difference, color='red', linestyle='dotted', linewidth=3)
#plt.axvline(total_average, color='black', linestyle='dotted', linewidth=3)
#plt.show()